...
save_plot(g1 ,"figs/Question_1_threshold.png", width=666)
@

<<>>=
g2 <- ggplot(example.height, aes(x=x, y=y)) 
g2 <- g2 + geom_area(aes(y=sigma_probs),fill="red", alpha=.4) 
g2 <- g2 + xlab("Threshold") + ylab("Adjective degree") + theme_gray(20) 
g2

save_plot(g2 ,"figs/Question_2_sigma.png", width=666)
@
...
# we add the result of updated belief
g1 <- g1 + geom_area(aes(y=es),fill="steelblue", alpha=.4) 
#g1 <- g1 + geom_area(aes(y=sigma_probs),fill="red", alpha=.4)
g1 <- g1 + xlab("IQ") + ylab("P") + theme_gray(20) 


g1

save_plot(g1 ,"figs/Question_2_IQ.png", width=666)
@
