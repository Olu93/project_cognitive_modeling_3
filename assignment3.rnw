...
densityModel = function(x){dnorm(x,180,10)}
cumulativeModel = function(x){dnorm(x,180,10)}
...
<<>>=
all.adjectives <- subset(data.adjective, Adjective != "dark" & Distribution != "right")
@
...
likelihood <- function(param1) {
    ...
    for (i in 1:14) {
        collect  <- collect + dnorm(all.adjectives$percentage[i], mean=use.adjective(i, adjective.points, param1[2], param1[1], densityModel, cumulativeModel), sd=0.1, log=TRUE)
