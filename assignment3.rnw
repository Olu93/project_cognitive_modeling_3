...
#lambda = function(x) {dunif(x, 1, 50)}
#coverage.pararemeter = function(x) {dunif(x, -1, 1)}

#densityModel = function(x){dnorm(x, , )}
#cumulativeModel = function(x){dnorm(x, , )}
...
likelihood <- function(param1) {
    ...
    for (i in 1:14) {
        #collect  <- collect + dnorm(data.gaus.big$percentage[i], mean= use.adjective(data.gaus.big$percentage[i], adjective.points, lambda, coverage.parameter, densityModel, cumulativeModel ), sd=0.1, log=TRUE)
